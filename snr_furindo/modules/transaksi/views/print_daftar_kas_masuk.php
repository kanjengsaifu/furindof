...
<label style="font-family:courier; font-size:18px; ">BUKU KAS HARIAN</label><br>
    ...
    <tbody name="tabelContent" id="tabelContent">
      <?php 
        $kas= $this->db->query("SELECT trx_kas.tgl_kas,trx_kas_det.memo,trx_kas.nomor_kas,trx_kas_det.nominal,trx_kas.jenis 
            from trx_kas_det inner join trx_kas on trx_kas.id_kas=trx_kas_det.id_kas where trx_kas.id_kasbank=2");
        $saldo_awal = $this->db->query("SELECT nominal from trx_jurnal where akun = '1.1.1'")->row();

...
<div class="content-header">        
	<h1>Edit Bukti Kas Masuk</h1>
</section>
<div class="content">        
	<div class="box box-primary">
		<div class="box-body">
			<form id="addkasmasuk" onsubmit="simpanreg(); return false;">
      <input type="hidden" name="id_jurnal" value="<?php echo $bkk->id_jurnal; ?>"/>
				...
				<div class="row">
					<div class="col-sm-6">
						...
						<label for="Nomor" class="col-sm-3 control-label">Nomor BKK :</label>
							<div class="col-sm-8">
								<input class="form-control" readonly id="nomor" name="nomor" value="<?php echo $bkk->nobukti; ?>"/>
							</div>
						</div>
                  ...
                  <input type="text" readonly role="date" value="<?php echo date('d-m-Y', strtotime($bkk->tgl)); ?>" class="form-control" id="tanggal" name="tanggal" >
            ...
            <label for="Nomor" class="col-sm-3 control-label">Jenis Transaksi :</label>
              <div class="col-sm-8">
                <select id="transaksi" onchange="Show_rekanan('')" class="form-control">
                  ...
                  <option selected="selected" value="umum">UMUM</option>
                </select>
              </div>
            </div>
                    ...
                    <input type="text" readonly value="Umum" class="form-control" id="rekanan" name="rekanan" >                    
            ...
            <?php }else { ?>
            <input type="hidden" id="id_rekanan" value="<?php echo $bkk->provider_id; ?>" name="id_rekanan" >
            <div class="form-group">
            <label for="Nomor" class="col-sm-3 control-label">Jenis Transaksi :</label>
              <div class="col-sm-8">
                <select id="transaksi" onchange="Show_rekanan('')" class="form-control">
            ...
            <div class="form-group show" id="input_rekanan">
            <label for="kegiatan" class="col-sm-3 control-label">Dibayar ke :</label>
              <div class="col-sm-8" id="col-kontak">
                <div class="input-group">
                    <input type="text" readonly value="<?php echo $bkk->provider_name; ?>" class="form-control" id="rekanan" name="rekanan" >                    
                      <span class="input-group-btn">
                        <button type="button" id="btnCariRekanan" class="btn btn-success"><span class="glyphicon glyphicon-search" aria-hidden="true"></span>&nbsp;Cari</button>
                      </span>
                    </div>
            ...
            <?php } ?>
						...
						<label for="Nomor" class="col-sm-3 control-label">Catatan :</label>
							<div class="col-sm-8">
								...
								<textarea class="form-control" id="catatan" name="catatan"><?php echo $bkk->memo; ?></textarea>
							</div>
						</div>
            ...
            <?php 
              $data_bkk = $this->db->query("SELECT * from trx_jurnal where id_induk = '".$bkk->id_jurnal."'");
              $no=0;
              foreach ($data_bkk->result() as $row) {
                if ($row->nominal < 0) {
                  $nilai = $row->nominal*-1; 
                }else{
                  $nilai = $row->nominal; 
            ...
            ?>
              <input type="hidden" name="jurnal_id[]" value="<?php echo $row->id_jurnal; ?>"/>
              <tr id='tmbinput-<?php echo $no; ?>'>
                <td><input type='text' readonly='readonly' value='<?php echo $row->akun; ?>' name='kode[]' id='kode-<?php echo $no; ?>' class='form-control autocomplate'/></td>
                <td><input type='text' readonly='readonly' value='<?php echo $row->uraian; ?>' name='uraian[]' id='uraian-<?php echo $no; ?>' class='form-control'/></td>
                <td><input type='text' style='text-align:right;'name='nominal[]' value="<?php echo number_format($nilai); ?>" id='nominal-<?php echo $no; ?>' onblur='calculates()' onkeyup='getnumeric(this)' class='form-control'/></td>
                <td><input type='text' name='memo[]' value="<?php echo $row->memo; ?>" id='memo-<?php echo $no; ?>' class='form-control'/></td>
                <td style='text-align:center;'><button type='button' onclick='deleterow(<?php echo $no; ?>)' id='<?php echo $no; ?>' title='hapus data' class='btn btn-xs btn-danger'><span class='glyphicon glyphicon-minus-sign'></span></button></td>
              </tr>
            <?php $no++; } ?>							
						</tbody>
						<tfoot>
					...
					<div class="row">
						...
						<div class="col-sm-5">
							...
							<label for="Nomor" class="col-sm-3 col-sm-offset-1 control-label">Masuk Ke :</label>
								<div class="col-sm-7">									
									<div class="input-group">
                      <input type="text" readonly value="<?php echo $bkk->uraian; ?>" class="form-control" id="kasbank" name="kasbank" >
                      <input type="hidden" value="0" id="id_kasbank" name="id_kasbank" >
                      <input type="hidden" value="<?php echo $bkk->akun; ?>" id="bank_code" name="bank_code" >
									   <span class="input-group-btn">
			...
			<div class="form-horizontal footer">
				<div class="row" id="addcol">
					<div class="col-sm-6" style="padding-left: 30px;">
						...
						<!-- <button type="submit" onclick="setTimeout(dialogFormPrint(<?php echo $bkk->id_jurnal; ?>),30000)" class="btn btn-sm btn-info"><span class="glyphicon glyphicon-print"></span> Simpan Data dan Cetak</button>
						<button type="button" onclick="batal()" class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-minus-sign"></span> Batal</button> -->
					</div>
				</div>		
...
function loadGridData(tipe){
	...
	{
		var target = "<?php echo site_url("transaksi/updatebkm")?>";

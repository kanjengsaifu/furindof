...
<div class="content-header">        
	<h1><a type="button" style="cursor:pointer" onclick="reload()">Input Hutang / Deposit</a></h1>
</section>
<div class="content" style="margin: 0px -35px">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
		    ...
		    <div class="form-group">
			    <label for="kodeKaryawan" class="col-sm-4 control-label">Nominal</label>
			    <div class="col-sm-8">
		    	  	<input type="text" onkeyup='getnumeric(this)' placeholder="Nominal IDR" name="usd" id="idr" class="form-control"/> 	
			    </div>
		    </div>		    		          
	      </div>
	      <div class="modal-footer">
...
$(document).ready(function () {
	...
	$(".date").datepicker({
      format : "dd-mm-yyyy",
      //startDate : new Date('<?php echo date('Y-m-d', strtotime("-5 days"))?>'),
...
function Detail(idm){
		loadhtml = ajaxFillGridJSON('akuntansi/DetailPembayaran', {IDBidang : idm});
		//loadhtml = "<?php echo site_url("akuntansi/DetailPembayaran")?>";
		$(".content-wrapper").html(loadhtml);
	}

function Deposit(idm){
		loadhtml = ajaxFillGridJSON('akuntansi/DepositPembayaran', {IDBidang : idm});
		//loadhtml = "<?php echo site_url("akuntansi/DepositPembayaran")?>";
		$(".content-wrapper").html(loadhtml);
	}

function DetailSM(idm){
		loadhtml = ajaxFillGridJSON('akuntansi/DetailPembayaranSM', {IDBidang : idm});
		//loadhtml = "<?php echo site_url("akuntansi/DetailPembayaran")?>";
		$(".content-wrapper").html(loadhtml);
	}
...
function loadGridDataHutang(){
         ...
         {		
             ...
             dataFields: [
                  ...
                  { name: "nominal2", 	type: "string" },

                  { name: "action", 	type: "string" }

             ],

            url : "akuntansi/GetDaftarHutang",
...

        {
            ...
            columns: [
              ...
              { text: 'Provider', cellsAlign: 'left', align: 'center', dataField: 'uraian', width : '36%'},

              { text: 'Deposit', cellsAlign: 'right', align: 'center', dataField: 'nominal2', width : '18%'},

              { text: 'Hutang', cellsAlign: 'right', align: 'center', dataField: 'nominal', width : '18%'},

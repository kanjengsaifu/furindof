<?php
  ob_start();
?>
<page footer="page" style="color:#012473; font-weight: lighter;">

    <style type="text/css">
.tg  {border-collapse:collapse;border-spacing:0; border-color: #012473 !important;}
.tg td{font-family:courier, sans-serif;font-size:11px;padding:0px 2px;border-style:solid;border-width:1px;overflow:hidden;word-break:normal;border-color: #012473 !important;}
.tg th{font-family:courier, sans-serif;font-size:11px;font-weight:normal;padding:1px 1px;border-style:solid;border-width:1px;overflow:hidden;word-break:normal;border-color: #012473 !important; }
...
<table class="tg" width="100%" style="height:-25px;"  >
   ...
   <th colspan="1" style="border-style:none; font-size:11px; height:-10px;">
      <img src="<?php echo base_url().'/assets/images/logo11.png'?>" alt="Logo SNR Furindo"/>
    </th>   
    <th colspan="3" style="border-style:none; font-size:11px; height:-10px; font-weight: bold;">
    ...
    <th colspan="3" style="border-style:none; font-size:11px; padding-left:50px; font-weight: bold;">
  ...
  <tr>
      <td colspan="5" style="border-style:none;border-bottom:1px solid #03011F; font-size:11px;">
  ...
  <?php 
      $data = $this->db->query("SELECT * from trx_lpb_detail inner join mst_product on trx_lpb_detail.product_id = mst_product.product_id 
        inner join trx_purchase_order on trx_purchase_order.purchase_order_id = trx_lpb_detail.purchase_order_id 
        inner join trx_sales_order on trx_sales_order.sales_order_id = trx_purchase_order.sales_order_id  where lpb_id = '".$lpb."'");
      $i=1;
      $ttl=0;
      $jml=0;
      $price=0;
  ...
  <tr>
    ...
    <td class="tg-yw4l"><?php $price += $ttl; echo $row->purchase_order_code.' '.$row->sales_order_ref_no.' '.$row->lpb_detail_note ?></td>
  </tr>  
  <?php $i++; }?>
  <tr>
    <td colspan="3" style="text-align:right; padding:1px "><b>TOTAL</b></td>
  ...
  <tr> 
      <td style="border-style:none; padding:1px;"></td>    
      <td colspan="2" style="font-family:courier; border-style:none;border-bottom:0px solid black; text-align:left; font-size:12px; padding:1px;">
...
<p style="font-family:courier; font-size:12px;border-bottom:0px;">BANTUL, <?php  $tgl=date('d F Y', strtotime($data1->lpb_date)); echo $tgl; ?></p>

<label style="font-family:courier; font-size:12px; border-bottom:0px;">Prepared By,</label>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
.tg td{font-family:courier, sans-serif;font-size:12px;padding:3px 5px;border-style:solid;border-width:1px;overflow:hidden;word-break:normal;}
...
.tg .tg-yw4l{text-align:right;vertical-align:top; font-weight: bold;}
</style>
<?php 
  //$data1 = $this->db->query("SELECT * from trx_lpb inner join mst_provider on trx_lpb.provider_id = mst_provider.provider_id where lpb_id = '".$lpb."'")->row();
?>
...
<table class="tg" width="100%">
  ...
  <tr>
      <td colspan="7" style="border-style:none;border-bottom:0px solid #03011F; text-align:center; font-size:16px;">
        <b>QC CHECKING REPORT</b>
      </td>
  </tr> 

  <tr>
      <td colspan="5" style="border-style:none;border-bottom:1px solid #03011F; font-size:11px;font-weight: bold;">
       NOMOR : <?php echo $data1->lpb_code; ?>
      </td>
      <td colspan="2" style="border-style:none;border-bottom:1px solid #03011F; font-size:11px;font-weight: bold;">
       SURAT JALAN NO : 
      </td>
  </tr> 
  <tr>
  ...
  <?php 
      $data2 = $this->db->query("SELECT * from trx_lpb_detail inner join mst_product on trx_lpb_detail.product_id = mst_product.product_id 
        inner join trx_purchase_order on trx_purchase_order.purchase_order_id = trx_lpb_detail.purchase_order_id 
        inner join trx_sales_order on trx_sales_order.sales_order_id = trx_purchase_order.sales_order_id  where lpb_id = '".$lpb."'");
      $i=1;
      $ttl=0;
  ...
  <tr>
    ...
    <td class="tg-yw4l"><?php $ssl +=$tolak; echo $row->purchase_order_code ?></td>
  </tr>  
  <?php $i++; }?>
  <tr>
  ...
  <tr>      
      <td colspan="7" style="border-style:none;border-bottom:0px solid #03011F; text-align:left; font-size:14px;">
        <!-- <b>Terbilang : <?php echo $terbilang; ?></b> -->
      </td>
  </tr> 
</table>
<p style="font-family:courier; font-size:12px; font-weight: bold;">BANTUL, <?php  $tgl=date('d F Y', strtotime($data1->lpb_date)); echo $tgl; ?></p>

<label style="font-family:courier; font-size:12px; font-weight: bold;">Quality Control,</label>

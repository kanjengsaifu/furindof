...
<div class="content-header">        
	<h1>Edit LPB Suport Material Cast</h1>
</section>
<div class="content">        
				...
				<div class="table-responsive" style="width:99%; margin:0px auto;">     
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						...
						?>
							...
							<tr id='tmbinput-<?php echo $num ?>'><input type='hidden' value='<?php echo $kbt ?>' id='ttl-<?php echo $num ?>' name='ttl_harga[]'/>
							...
							<td><input type='number' value='<?php echo $row->lpb_liquid_detail_qty ?>' min='1' step='0.01' name='qty[]' id='qty-<?php echo $num ?>' oninput='getnumeric(this)' atm='nominal-<?php echo $num ?>' class='form-control autocomplate'/></td>
...
function loadGridData(){ 
		...
		//alert(idso);		 
        ajaxDataGrid('<?php echo base_url()?>lpb/addTableLiquid_bebas', {idx : produk_id, ids : idso}, 'tableGridData');
        $('#example3').dataTable({
          "bPaginate": true,
          "bLengthChange": true,
          "bFilter": true,
...
function addProduct(objReference, idx)
	{ 
		...
		var qty 		= $(objReference).parent().parent().find('td:eq(4)').html();
	 	...
	 	var htmlOut = ajaxFillGridJSON('lpb/adddataSMBebas', {lpb_id : idpo ,mat_id : Id ,price : price });
		$('#myModal').attr('class', 'modal hide');
		var htmlOut1 = ajaxFillGridJSON('lpb/edit_lpb_bebas', {IDBidang : idpo});	    		
...
function simpanreg()
	{
		...
		var htmlOut = ajaxFillGridJSON('lpb/updateLPBSMBebas', data);	    		
	   	//$('.content-wrapper').html(htmlOut);
	   	//return false;

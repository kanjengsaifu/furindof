				...
				<div class="table-responsive" style="width:99%; margin:0px auto;">     
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						...
						<?php
							...
							$i = $num + 1;
							$inv = $this->db->query("SELECT sum(purchase_order_detail_qty) as stok_qty from trx_purchase_order_detail where purchase_order_id = '".$row->purchase_order_id."' AND material_id = '".$row->material_id."'");
							$cek = $this->db->query("SELECT sum(lpb_detail_qty) as lpb_detail_qty from trx_lpb_detail inner join trx_lpb on trx_lpb_detail.lpb_id = trx_lpb.lpb_id  where purchase_order_id = '".$row->purchase_order_id."'  AND material_id = '".$row->material_id."'")->row();
						...
						?>
							...
							<td><input type='number' min='1' max='<?php echo $max ?>' value='<?php echo $row->lpb_detail_qty ?>' name='qty[]' id='qty-<?php echo $num ?>' onchange='getnumeric(this)' atm='nominal-<?php echo $num ?>' class='form-control autocomplate'/></td>
...
function addProduct(objReference, idx)
	{ 
		...
		var beli 		= $(objReference).parent().parent().find('td:eq(9)').html();
	 	...
	 	var htmlOut = ajaxFillGridJSON('lpb/adddataRaw', {lpb_id : idpo ,mat_id : Id ,prod_id : IDproduct ,price : price ,qty : qty, PO : IDPO1});
		$('#myModal').attr('class', 'modal hide');
		var htmlOut1 = ajaxFillGridJSON('lpb/edit_lpb_raw', {IDBidang : idpo});	    		
	...
	{		
		...
		isDelete = confirm('Yakin data akan dihapus ?');
	  	if (isDelete) sendRequestForm('lpb/habusdataRaw', {ID : obj, IDV : idv}, 'box-body');
	  	var htmlOut = ajaxFillGridJSON('lpb/edit_lpb_raw', {IDBidang : idpo});	    		
...
function simpanreg()
	{
		...
		var htmlOut = ajaxFillGridJSON('lpb/updateLPB', data);	    		
	   	//$('.content-wrapper').html(htmlOut);
	   	//return false;

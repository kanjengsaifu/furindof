<?php $tgl = date("d-m-Y", strtotime($LPB->lpb_date)); ?>
<style>
	.seperator{
...
<div class="content">        
	<div class="box box-primary">
		<div class="box-body">
			...
			<input type="hidden" id="lpbid" name="lpb_id" value="<?php echo $LPB->lpb_id ?>" required/>
				...
				<div class="row">
					...
					<div class="col-sm-6">
						...
						<label for="Nomor" class="col-sm-3 control-label">Biaya Kirim :</label>
							<div class="col-sm-8">
								<div class="input-group">
                                    <input onkeyup='getnumericid(this)' onblur='calculates()' type="text" value="<?php echo number_format($LPB->lpb_biaya) ?>" name="biaya" class="form-control" id="biaya" >
						...
						<label for="Nomor" class="col-sm-3 control-label">Note :</label>
							<div class="col-sm-8">
								<textarea class="form-control" name="note"><?php echo $LPB->lpb_note ?></textarea>
							</div>
						</div>
					</div>				
				...
				<div class="table-responsive" style="width:99%; margin:0px auto;">     
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						...
						<?php
							...
							$i = $num + 1;
							$inv = $this->db->query("SELECT sum(qty) as stok_qty from trx_purchase_order_sample where purchase_order_sample_id = '".$row->purchase_order_sample_id."' AND material_id = '".$row->material_id."'");
							$cek = $this->db->query("SELECT sum(lpb_detail_qty) as lpb_detail_qty from trx_lpb_sample_detail inner join trx_lpb_sample on trx_lpb_sample_detail.lpb_id = trx_lpb_sample.lpb_id  where purchase_order_sample_code = '".$row->purchase_order_sample_code."'  AND material_id = '".$row->material_id."'")->row();
							$stock = $inv->row()->stok_qty+0;
							$beli = $cek->lpb_detail_qty-$row->lpb_detail_qty;
							$max = $stock-$beli;
							$kbt = $row->lpb_detail_price * $row->lpb_detail_qty;
							
						?>
							<input type='hidden' value='<?php echo $row->lpb_detail_id ?>' name='iddetail[]'/>
							...
							<td><input type='text' value='<?php echo number_format($row->lpb_detail_price) ?>' name='nominal[]' id='nominal-<?php echo $num ?>' atm='nominal-<?php echo $num ?>' onkeyup='getnumeric(this)' class='form-control'/></td>
							<td><input type='number' readonly value='<?php echo $stock ?>' class='form-control autocomplate'/><input type='hidden' value='<?php echo $row->purchase_order_id ?>' name='id_PO[]'/></td>
							<td><input type='number' value='<?php echo $row->lpb_detail_datang ?>' name='datang[]' class='form-control autocomplate'/><input type='hidden' value='<?php echo $kbt ?>' id='ttl-<?php echo $num ?>' name='ttl_harga[]'/></td>
							<td><input type='number' min='0' max='<?php echo $max ?>' value='<?php echo $row->lpb_detail_qty ?>' name='qty[]' id='qty-<?php echo $num ?>' onkeyup='getnumeric(this)' atm='nominal-<?php echo $num ?>' class='form-control autocomplate'/></td>
							<td><input type='text' name='desc[]' id='desc-<?php echo $num ?>' value='<?php echo $row->lpb_detail_note ?>' class='form-control autocomplate'/></td>
							<td style='text-align:center;'><button type='button' onclick='deleterow(<?php echo $row->lpb_detail_id ?>,<?php echo $row->inventory_id ?>)' title='hapus data' class='btn btn-xs btn-danger'><span class='glyphicon glyphicon-minus-sign'></span></button></td>
							<!-- <td colspan='1' style='text-align:center;'><button type='button' onclick='addinfo(<?php echo $row->lpb_detail_id ?>)' title='info data' class='btn btn-xs btn-primary'><span class='glyphicon glyphicon-info-sign'></span></button></td> -->
							</tr>
...
function loadGridData(){ 
		...
		//alert(idso);		 
        ajaxDataGrid('<?php echo base_url()?>sample/addTablePO_det', {idx : produk_id, ids : idso}, 'tableGridData');
        $('#example3').dataTable({
          "bPaginate": true,
          "bLengthChange": true,
          "bFilter": true,
...
function addProduct(objReference, idx)
	{ 
		...
		var idpo = $('#lpbid').val();
	 	...
	 	var sample 		= $(objReference).parent().parent().find('td:eq(1)').html();
		...
		var IDPO1 		= $(objReference).parent().parent().find('td:eq(7)').html();
		var stock 		= $(objReference).parent().parent().find('td:eq(8)').html();
		var beli 		= $(objReference).parent().parent().find('td:eq(9)').html();
		
	 	var htmlOut = ajaxFillGridJSON('sample/adddataSample', {lpb_id : idpo ,mat_id : Id ,prod_id : IDproduct ,price : price ,qty : qty, PO : IDPO1, sample : sample});
		$('#myModal').attr('class', 'modal hide');
		var htmlOut1 = ajaxFillGridJSON('sample/edit_lpb_sample', {IDBidang : idpo});	    		
	...
	{		
		...
		isDelete = confirm('Yakin data akan dihapus ?');
	  	if (isDelete) sendRequestForm('sample/habusdataSample', {ID : obj, IDV : idv}, 'box-body');
	  	var htmlOut = ajaxFillGridJSON('sample/edit_lpb_sample', {IDBidang : idpo});	    		
...
function simpanreg()
	{
		...
		var htmlOut = ajaxFillGridJSON('sample/updateLPBSample', data);	    		
	   	//$('.content-wrapper').html(htmlOut);
	   	//return false;
		//sendRequestForm('admin/updateso', data, 'box-body');
		var kodeTipeKaryawan = ajaxFillGridJSON('sample/lpb_sample', {IDBidang : idx}); 

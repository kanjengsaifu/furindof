...
<div class="content-header">        
	<h1>Import Suport Material</h1>
</section>
<div class="content">        
	<div class="box box-primary">
		<div class="box-body">
			<form id="addkso" onsubmit="simpanreg(); return false;">
			<input type="hidden" id="po_id" name="po_id" value="" required/>
					...
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						<thead>
							<tr role="row">
								...
								<th class="btn-primary" style="width:32%; text-align:center; vertical-align: middle;">Material Name</th>
						...
						<?php 	
						 	$file_path ='assets/export/suport.xls';          
							...
							$num = $no-1;
							$material = $this->db->query("SELECT * from mst_material where material_code = '".$row['B']."'")->row();
						?>	
							<input type='hidden' value='' name='iddetail[]'/>
							<tr id='tmbinput-<?php echo $num ?>'>
							<td><?php echo $no ?><input type='hidden' value='<?php echo $material->material_id ?>' name='id_material[]'/><input type='hidden' value='<?php echo $row->material_id ?>' name='id_product[]'/></td>
							<td><input type='text' readonly='readonly' value='<?php echo $material->material_code ?>' id='uraian-<?php echo $num ?>' class='form-control'/></td>
							<td><input type='text' value='<?php echo $material->material_name ?>' style='text-align:left;' id='nmproduct-<?php echo $num ?>'class='form-control'/></td>
							<td><input type='text' value='<?php echo number_format($row['D']) ?>' name='nominal[]' id='nominal-<?php echo $num ?>' onkeyup='getnumeric(this)' class='form-control'/></td>
							<td><input type='number' readonly value='<?php echo $stock ?>' class='form-control autocomplate'/></td>
							<td><input type='number' readonly value='0' class='form-control autocomplate'/></td>
							<td><input type='number' step="0.001" min='1' value='<?php echo $row['C'] ?>' name='qty[]' id='qty-<?php echo $num ?>' class='form-control autocomplate'/></td>
							<td><input type='text' name='desc[]' id='desc-<?php echo $num ?>' value='<?php echo $row['E'] ?>' class='form-control autocomplate'/></td>
							<td style='text-align:center;'><button type='button' onclick='deleterow()' title='hapus data' class='btn btn-xs btn-danger'><span class='glyphicon glyphicon-minus-sign'></span></button></td>
		...
		<table id="example3" class="table table-bordered table-striped">
            <thead style="">
                <tr>            
                    ...
                    <th class="hidden" tabindex='4' style="text-align:center; width:14%"> Kebutuhan </th> 
...
function loadGridData(){ 
        ...
        ajaxDataGrid('<?php echo base_url()?>suport/addTableSo_lc', {idx : produk_id, ids : idso, idp : idpo}, 'tableGridData');
        $('#example3').dataTable({
          "bPaginate": true,
          "bLengthChange": true,
          "bFilter": true,

<?php $tglreg = $ship->shipment_date; $tgl = $ship->shipment_loading_date; ?>
<style>
	.seperator{
...
<div class="content">        
	<div class="box box-primary">
		<div class="box-body">
			...
			<input type="hidden" id="soid" name="so_id" value="<?php echo $ship->shipment_id ?>" required/>
				...
				<div class="row">
					<div class="col-sm-6">
						...
						<label for="Nomor" class="col-sm-3 control-label">NO Shipment :</label>
							<div class="col-sm-8">
								<input class="form-control" id="nomor" name="nomor" value="<?php echo $ship->shipment_code ?>" required/>
							</div>
						</div>
						...
						<label for="Nomor" class="col-sm-3 control-label">Currency :</label>
							<div class="col-sm-8">
								<div class="input-group">
                                    <input type="text" value="<?php echo $ship->shipment_rate_currency ?>" role="date" class="form-control" id="Currency" name="Currency" >
					...
					<div class="col-sm-6">
						...
						<label for="Nomor" class="col-sm-3 control-label">Container Code :</label>
							<div class="col-sm-8">
								<input class="form-control" id="Container" name="Container" value="<?php echo $ship->shipment_container_code ?>" required/>
							</div>
						</div>
						<div class="form-group">
						<label for="Nomor" class="col-sm-3 control-label">Driver :</label>
							<div class="col-sm-8">
								<input class="form-control" id="Driver" name="Driver" value="<?php echo $ship->shipment_driver ?>" required/>
							</div>
						</div>
						<div class="form-group">
						<label for="Nomor" class="col-sm-3 control-label">Truck Code :</label>
							<div class="col-sm-8">
								<input class="form-control" id="Truck" name="Truck" value="<?php echo $ship->shipment_truck_code ?>" required/>
							</div>
						</div>
						<div class="form-group">
						<label for="Nomor" class="col-sm-3 control-label">Seal Code :</label>
							<div class="col-sm-8">
								<input class="form-control" id="Seal" name="Seal" value="<?php echo $ship->shipment_seal_code ?>" required/>
							</div>
						</div>						
			...
			<div class="header1">
				...
				<div class="table-responsive" style="width:99%; margin:0px auto;">     
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						...
						<?php 
							$data = $this->db->query("SELECT * from trx_shipment_detail inner join mst_product on mst_product.product_id = trx_shipment_detail.product_id where shipment_id = '".$ship->shipment_id."'");
							$no=1;
							foreach ($data->result() as $row) {
								$volume = $row->shipment_detail_cmb * $row->shipment_detail_qty;
								$wg = $row->shipment_detail_kg * $row->shipment_detail_qty;
								$ph = $row->shipment_detail_price * $row->shipment_detail_qty;
								
						?>
							<tr id='tmbinput-<?php echo $no ?>'><input type='hidden' value='<?php echo $row->shipment_detail_id ?>' name='idx[]'/>
								<td><?php echo $no ?><input type='hidden' value='<?php echo $row->sales_order_id ?>' name='id_material[]'/><input type='hidden' value='<?php echo $row->product_id ?>' name='id_product[]'/></td>
								<td><input type='text' readonly value='<?php echo $row->product_code ?>' style='text-align:left;' id='codeproduct-<?php echo $no ?>'class='form-control'/></td>			
								...
								<td><input type='text' readonly value='<?php echo $row->shipment_detail_cmb ?>' style='text-align:left;' name='cmb[]' id='cmbproduct-<?php echo $no ?>' class='form-control'/></td>
								<td><input type='text' readonly value='<?php echo $row->shipment_detail_kg ?>' style='text-align:left;' name='kg[]' id='kgproduct-<?php echo $no ?>'class='form-control'/></td>
								<td><input type='text' readonly value='<?php echo $row->shipment_detail_price ?>' style='text-align:left;' name='price[]' id='prproduct-<?php echo $no ?>'class='form-control'/></td>
								<td><input type='number' min='1' max='' value='<?php echo $row->shipment_detail_qty ?>' name='qty[]' class='form-control'/></td><input type='hidden' value='<?php echo $row->shipment_detail_bdl ?>' name='bdl[]'/>
								<td><input type='text' readonly value='<?php echo $volume ?>' name='volume[]' class='form-control'/></td>
								<td><input type='text' readonly value='<?php echo $wg ?>' name='weight[]' class='form-control'/></td>
								<td><input type='text' readonly value='<?php echo $ph ?>' name='usd[]' class='form-control'/></td>
								<td style='text-align:center;'><button type='button' onclick='deleteship(<?php echo $row->shipment_detail_id ?>)' id='"+idbutton+"' title='hapus data' class='btn btn-xs btn-danger'><span class='glyphicon glyphicon-minus-sign'></span></button></td>
							</tr>
						<?php 
						$no++;
						}
...
$(document).ready(function(){
		loadGridData();
		var tglreg = "<?php echo date("d-m-Y", strtotime($tglreg))?>";
		var tgl = "<?php echo date("d-m-Y", strtotime($tgl))?>";
		$("#tglreg").val(tglreg);
		$("#loading").val(tgl);		
...
function addProduct(objReference, idx)
	...
	function deleteship(obj)
	{		
		var idpo = $('#soid').val();
		//alert(idpo);
		//return false;
		isDelete = confirm('Yakin data akan dihapus ?');
	  	if (isDelete) sendRequestForm('production/habusdataship', {ID : obj}, 'box-body');
	  	var htmlOut = ajaxFillGridJSON('production/edit_shipment', {IDBidang : idpo});	    		
	   	$('.content-wrapper').html(htmlOut);
...
function simpanreg()
	{
		...
		var htmlOut = ajaxFillGridJSON('production/updateShipment', data);	    		
	   	//$('.content-wrapper').html(htmlOut);
	   	//return false;

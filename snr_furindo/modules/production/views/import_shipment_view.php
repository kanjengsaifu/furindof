			...
			<div class="header1">
				...
				<div class="table-responsive" style="width:99%; margin:0px auto;">     
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						...
						<?php 	
						 	$file_path ='assets/export/shipment.xls';          
							...
							$material1 = $this->db->query("SELECT trx_packing_detail.*, mst_product.*, sum(packing_detail_qty) as qty from trx_packing_detail 
								inner join mst_product on mst_product.product_id = trx_packing_detail.product_id where product_code = '".$row['B']."'
								group by trx_packing_detail.product_id");
							$material = $material1->row();
							if($material1->num_rows() == 0){

							}else{
								$volume = $material->product_cbm*$row['C'];
								$price = $material->packing_nominal*$row['C'];
								$weight = $material->product_weight*$row['C'];
							}
						?>	
							<tr id='tmbinput-<?php echo $no; ?>'>
							<td><?php echo $no; ?><input type='hidden' value='<?php echo $material->material_id ?>' name='id_material[]'/><input type='hidden' value='<?php echo $material->product_id ?>' name='id_product[]'/></td>
							<td><input type='text' readonly value='<?php echo $row['B'] ?>' style='text-align:left;' id='codeproduct-<?php echo $no; ?>'class='form-control'/></td>		
							<td><input type='text' readonly value='<?php echo $material->product_name ?>' style='text-align:left;' id='nmproduct-<?php echo $no; ?>'class='form-control'/></td>
							<td><input type='text' readonly value='<?php echo $material->product_cbm ?>' style='text-align:left;' name='cmb[]' id='cmbproduct-<?php echo $no; ?>' class='form-control'/></td>
							<td><input type='text' readonly value='<?php echo $material->product_weight ?>' style='text-align:left;' name='kg[]' id='kgproduct-<?php echo $no; ?>'class='form-control'/></td>
							<td><input type='text' readonly value='<?php echo $material->packing_nominal ?>' style='text-align:left;' name='price[]' id='prproduct-<?php echo $no; ?>'class='form-control'/></td>
							<td><input type='text' min='1' max='' value='<?php echo $row['C']; ?>' name='qty[]' class='form-control'/></td><input type='hidden' value='"+bdl+"' name='bdl[]'/>
							<td><input type='text' readonly value='<?php echo $volume; ?>' name='volume[]' class='form-control'/></td>
							<td><input type='text' readonly value='<?php echo $weight ?>' name='weight[]' class='form-control'/></td>
							<td><input type='text' readonly value='<?php echo $price; ?>' name='usd[]' class='form-control'/></td><input type='hidden' value='' name='pack[]' class='form-control'/>

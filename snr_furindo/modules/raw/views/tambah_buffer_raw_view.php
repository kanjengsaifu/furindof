					...
					<table id="tables"  width="100%" cellspacing="0" aria-describedby="tabel transaksi" role="grid" class="table table-striped table-bordered">
						<thead>
							<tr role="row">
								...
								<th class="btn-primary" style="width:12%; text-align:center; vertical-align: middle;">Stock</th>
...
function loadGridData(){ 
		...
		var idso = $('#id_sales').val();		 
        ajaxDataGrid('<?php echo base_url()?>raw/addTableSo_det_buffer', {idx : produk_id, ids : idso}, 'tableGridData');
        $('#example3').dataTable({
          "bPaginate": true,
          "bLengthChange": true,
          "bFilter": true,
...
function loadGridDataSO(){ 
        ...
        ajaxDataGrid('<?php echo base_url()?>raw/addTableSalesBuffer', {idx : produk_id}, 'tableGridDataSales'); 
        $('#example5').dataTable({
          "bPaginate": true,
          "bLengthChange": true,
          "bFilter": true,
...
function addProduct(objReference, idx)
	{ 
		if(idx == 0){
			alert('Stock material tidak ada !');
			return false;
		}
		...
		var idbutton = "delete-button-"+$("#tabelContent tr").length;
			...
			var row  = "<tr id='tmbinput-"+lengths+"'>";
				...
				row += "<td><input type='text' value='"+stock+"' name='stock[]' id='stock-"+lengths+"' class='form-control'/></td><input type='hidden' value='"+harga+"' name='price[]' id='price-"+lengths+"'/>";
				row += "<td><input type='number' min='1' max='"+stock+"' value='"+qty+"' name='qty[]' id='qty-"+lengths+"' class='form-control autocomplate'/></td>";
...
function simpanreg()
	{
		...
		var htmlOut = ajaxFillGridJSON('raw/saveBuffer', data);	    		
	   	//$('.content-wrapper').html(htmlOut);
	   //return false;
